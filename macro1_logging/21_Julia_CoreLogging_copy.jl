...
export
    ...
    @error,
    @logmsg,
    # with_logger, # 和Logging重复
    current_logger,
    # global_logger, # 和Logging重复
    disable_logging,
    SimpleLogger
...
function logmsg_code(_module, file, line, level, message, exs...)
    ...
    return quote
        let
            ...
            std_level = $(:level)
            # std_level = $(level isa Symbol ? :level : :(level isa $(Main.CoreLogging.LogLevel) ? level : convert($(Main.CoreLogging.LogLevel), level)::$LogLevel))
            # std_level = $(level isa Symbol ? :level : :Info)
            display("std_level")
            display(std_level)
            ($(Main.CoreLogging._min_enabled_level)[]) |>display
            
            if std_level >= $(Main.CoreLogging._min_enabled_level)[]
